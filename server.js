...
function createComment(url, request) {
  ...
  if (requestComment && Object.keys(requestComment).length >= 1 && (requestComment.body) && requestComment.username &&
...
function updateComment(url, request) {
  ...
  if (!id || !savedComment) {
    ...
    // savedComment.articleId = requestComment.articleId;
    // savedComment.username = requestComment.username;
...
function loadDatabase() {
  database = yaml.read('database.yaml',{encoding: "utf8", schema: yaml.schema.defaultSafe});
  return database;
}
